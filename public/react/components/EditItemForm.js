...
export const EditItem = () => {
    const {isEditClicked,items,setIsEditClicked,itemUpdate,setItemUpdate,updateItem,selectedId,isItemListDisplay,setIsItemListDisplay,detailViewClicked,setDetailViewClicked}=React.useContext(AppContext)
    ...
    function onChangeHandler(event){
      ...
      return <>
        {items.map((indvItem, indx)=>{
            if(indvItem.id===selectedId){
                return <div className='SingleItemEditForm'> 
                <form className='Editform' onSubmit={onsubmitHandler} key={indx} >
